  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        setSuccess(true);
        // Automatically check if they are logged in directly (e.g. autoConfirm enabled in Supabase)
        // or if they need to check their email first.
        setTimeout(() => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color: 'var(--text-2)', fontSize: '0.875rem', marginBottom: '2rem' }}>
          Join the Vichith community and claim your creator URL.
        </p>
        ...
        {success && (
          <div style={{
            ...
            Account created successfully! Directing to onboarding...
          </div>
        )}
        ...
        <p style={{ color: 'var(--text-2)', fontSize: '0.85rem', marginTop: '1.75rem', margin: '1.75rem 0 0 0' }}>
          Already have an account? <a href="/login" style={{ color: 'var(--cyan)', textDecoration: 'none', fontWeight: 600 }}>Log in</a>
        </p>
      </div>
    </div>
  );
