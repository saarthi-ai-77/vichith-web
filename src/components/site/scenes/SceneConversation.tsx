  ...
  useEffect(() => {
    function onCamera(e: Event) {
      ...
      BEATS.forEach((beat, i) => {
        ...
        // Cap blur at 3px — filter:blur forces GPU resampling every frame,
        // so keeping it minimal significantly reduces compositor cost.
        if (blurPx > 0.1) {
          inner.style.filter = `blur(${Math.min(blurPx, 3).toFixed(1)}px)`;
        } else {
          inner.style.filter = "";
        }
        ...
        if (beat.kind === "user" || beat.kind === "chithra") {
          ...
          if (el) {
            // Cap blur at 6px (was 14px) — large blur values are very expensive
            // for the GPU compositor when animated every scroll frame.
            const blur = Math.min(6, 6 * (1 - local));
            const sat = 0.3 + local * 0.9;
            el.style.filter = blur > 0.1
