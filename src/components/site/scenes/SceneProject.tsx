...
export function SceneProject() {
  ...
  useEffect(() => {
    const tween = gsap.to(gridRef.current, {
      ...
      yoyo: true,
      ease: "sine.inOut",
      paused: true, // start paused; only run when camera is nearby
    });

    function onCamera(e: Event) {
      const { cameraZ } = (e as CustomEvent<{ cameraZ: number }>).detail;
      const projectZ = Math.abs(DEPTH.project);
      const near = Math.abs(cameraZ - projectZ) < 2500;
      if (near && tween.paused()) tween.play();
      else if (!near && !tween.paused()) tween.pause();
    }

    window.addEventListener(CAMERA_EVENT, onCamera);
    return () => {
      window.removeEventListener(CAMERA_EVENT, onCamera);
      tween.kill();
    };
  }, []);
