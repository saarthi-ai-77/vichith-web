...
// Using the same technique as SceneConversation / SceneEcosystem.
...
export function GlobalMatrix() {
  ...
  // Pre-computed, deterministic scatter. 28 items instead of 40 — fewer
  // GPU layers compositing simultaneously is the single biggest scroll
  // smoothness win on low-end hardware, and the visual density stays fine.
  const items = useMemo(() => {
    const totalItems = 28;
    const array = [];
    ...
    for (let i = 0; i < totalItems; i++) {
      const seed = i * 7.3;
      const z = 500 - pr(seed + 1.1) * (SCROLL_DISTANCE + 1000);

      let x = pr(seed + 2.2) * 200 - 50;
      let y = pr(seed + 3.3) * 140 - 20;

      // Push elements away from dead center to keep text readable
      if (x > 30 && x < 70) x += pr(seed + 4.4) > 0.5 ? 40 : -40;
      if (y > 30 && y < 70) y += pr(seed + 5.5) > 0.5 ? 40 : -40;
      ...
      array.push({
        ...
        type: asset.type,
        src: asset.src,
        width: 250 + pr(seed + 6.6) * 150,
        height: 150 + pr(seed + 7.7) * 100,
        rotation: pr(seed + 8.8) * 10 - 5,
      });
    }
